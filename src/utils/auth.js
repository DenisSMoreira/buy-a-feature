...
export default (ctx = {}) => {
  ...
  api.interceptors.request.use((config) => {
    ...
    if (!config.headers.Authorization) {
      ...
      config.headers.Authorization = token ? `Bearer ${token}` : `Basic ${AUTHORIZATION_BEARER}`;
    }
    ...
    async (error) => {
      if (error?.response?.status === 401) {
        ...
        if (!originalRequest.retry) {
          ...
          if (refreshToken) {
            try {
              const { data } = await api.post('/oauth/token', qs.stringify(refreshHydrate(refreshToken)), {
                headers: {
